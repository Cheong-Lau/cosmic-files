...
use crate::{
    ...
    tab::{self, Item, ItemMetadata, Location, SearchLocation, Tab},
...
impl<M: Send + 'static> Dialog<M> {
    ...
    pub fn set_title(&mut self, title: impl AsRef<str>) -> Task<M> {
        let mapper = self.mapper;
        title.as_ref().clone_into(&mut self.cosmic.app.title);
        self.cosmic
            .app
            .update_title()
            .map(DialogMessage)
        ...
        for (choice_i, choice) in self.choices.iter().enumerate() {
            match choice {
                DialogChoice::CheckBox { label, value, .. } => {
                    row =
                        row.push(widget::checkbox(label.clone(), *value).on_toggle(
                            move |checked| Message::Choice(choice_i, usize::from(checked)),
                        ));
                }
                DialogChoice::ComboBox {
                    ...
                    row = row
                        .push(widget::text::heading(label.clone()))
            ...
            PreviewKind::Selected => {
                if let Some(items) = self.tab.items_opt() {
                    ...
                    if children.is_empty()
                        && let Some(item) = self.tab.parent_item_opt.as_ref()
                    {
                        children.push(item.preview_view(None, military_time));
                    }
                }
                    ...
                    match self.flags.config.type_to_search {
                        ...
                        TypeToSearch::EnterPath => {
                            let location = (self.tab.edit_location)
                                .as_ref()
                                .map_or(&self.tab.location, |x| &x.location);
                        ...
                        items.retain(|item| {
                            ...
                            item.metadata.is_dir()
                                // Check for mime type match (first because it is faster)
                                    || mimes.iter().any(|&mime| mime == item.mime)
