...
fn dir_info(uri: &str) -> Result<(String, Box<str>, Option<PathBuf>), glib::Error> {
...
enum Cmd {
    ...
    DirInfo(
        String,
        mpsc::Sender<Result<(String, Box<str>, Option<PathBuf>), glib::Error>>,
    ),
    Unmount(MounterItem),
}
...
impl Mounter for Gvfs {
    ...
    fn mount(&self, item: MounterItem) -> Task<anyhow::Result<()>> {
        ...
        Task::future(async move {
            ...
            res_rx
                .await
                .expect("Expected mount command to return result")
        })
    }
    ...
    fn network_drive(&self, uri: String) -> Task<anyhow::Result<()>> {
        ...
        Task::future(async move {
            ...
            res_rx
                .await
                .expect("Expected network drive command to return result")
        })
    }
    ...
    fn dir_info(&self, uri: &str) -> Option<(String, Box<str>, Option<PathBuf>)> {
